# -*- coding: utf-8 -*-
"""
Lightweight HTTP WebIF server — screenshot & recording control from
any browser on the LAN. No external dependencies beyond stdlib.
...
:root,[data-theme="light"]{
  --bg:#f7f6f2;--surface:#f9f8f5;--surface-2:#fff;--border:#d4d1ca;
  --text:#28251d;--muted:#7a7974;--primary:#01696f;--primary-h:#0c4e54;
  --error:#a12c7b;--success:#437a22;--rec:#cc2222;
  --shadow:0 4px 16px rgba(0,0,0,.08);
  --radius:0.6rem;--font:'Segoe UI',system-ui,sans-serif;
}
[data-theme="dark"]{
  --bg:#111110;--surface:#1c1b19;--surface-2:#242320;--border:#393836;
  --text:#cdccca;--muted:#797876;--primary:#4f98a3;--primary-h:#227f8b;
  --error:#d163a7;--success:#6daa45;--rec:#ff4444;
  --shadow:0 4px 24px rgba(0,0,0,.4);
}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
html{-webkit-font-smoothing:antialiased;scroll-behavior:smooth}
body{font-family:var(--font);font-size:15px;color:var(--text);
  background:var(--bg);min-height:100dvh;padding:0}
...
button{cursor:pointer;border:none;border-radius:var(--radius);
  font:inherit;font-size:14px;padding:8px 16px;transition:.18s ease}
...
.rec-badge{display:inline-flex;align-items:center;gap:6px;
  background:rgba(204,34,34,.15);color:var(--rec);
  border:1px solid rgba(204,34,34,.3);border-radius:999px;
...
.fmt-btn{padding:6px 14px;border-radius:999px;font-size:13px;
...
.capture-item{display:flex;align-items:center;justify-content:space-between;
  ...
  border-radius:calc(var(--radius)*.7);padding:10px 14px;gap:10px}
.cap-info{flex:1;min-width:0}
.cap-name{font-size:13px;font-weight:500;white-space:nowrap;
...
.toast{position:fixed;bottom:20px;right:20px;background:var(--surface);
  ...
  padding:12px 18px;box-shadow:var(--shadow);font-size:13px;
  transform:translateY(80px);opacity:0;transition:.3s ease;z-index:999;
  max-width:300px}
...
@media(max-width:600px){
  .actions-row button{flex:1;justify-content:center;text-align:center}
